...
shopify.ProductImporter = class {
	...
	async fetchProductCount() {
		try {
			const {
				...
				method: "shopify_integration.shopify.page.shopify_import_products.shopify_import_products.get_product_count",
			});
	...
	async syncProduct(product) {
		const { message: status } = await frappe.call({
			method: "shopify_integration.shopify.page.shopify_import_products.shopify_import_products.sync_product",
			args: { product },
		});
	...
	async resyncProduct(product) {
		const { message: status } = await frappe.call({
			method: "shopify_integration.shopify.page.shopify_import_products.shopify_import_products.resync_product",
			args: { product },
		});
	...
	syncAll() {
		...
		if (this.syncRunning) {
			...
			frappe.call({
				method: "shopify_integration.shopify.page.shopify_import_products.shopify_import_products.import_all_products",
			});
		}
