...
def sync_sales_order(payload, request_id=None):
	...
	try:
		...
		# When backfilling an order that was already cancelled in Shopify,
		# reflect that by cancelling the just-created Sales Order too.
...
@temp_shopify_session
def sync_old_orders_any_state():
	shopify_setting = frappe.get_cached_doc(SETTING_DOCTYPE)
	if not cint(shopify_setting.sync_old_orders_any_state):
		return
	...
	shopify_setting = frappe.get_doc(SETTING_DOCTYPE)
	shopify_setting.sync_old_orders_any_state = 0
	shopify_setting.save()


def _fetch_old_orders(from_time, to_time, limit=50):
	# Runs only as a cron-scheduled job (see hooks.py scheduler_events),
	# never in a request context with a logged-in user.
	frappe.set_user("Administrator")  # nosemgrep: security.frappe-setuser
